...
We follow [A, B, C, D, E] and consider an attacker without access to the training dataset but with white-box access to a trained text-to-image diffusion model.object_erasure_samples
...
encodings (TextEnc), distances between the produced keyand value projections in the cross-attention layers (X-Attn),
or between predicted scores affecting the entire denoising network (DISA). For more details on the TextEnc and X-Attn attacks, please refer to the paper.
<Figure>
  <Image slot="figure" source={toxe_parameter_subsets} altText="" />
  <span slot="caption">**Parameter Subsets fine-tuned by different (ToxE) attacks.** </span>
</Figure>


## DISA: Deep Intervention Score-based Attack

In contrast to the other two attacks, DISA does not only manipulate the text encoder or cross-attention layers but optimizes the entire U-Net. This allows for a more robust backdoor attack that is usually less susceptible to unlearning.

The objective has three terms: (1) the trigger loss that brings the score predictions of the trigger concept close to the target concept, a (2) regularization term that explicitly preserves (optionally) specified retention concepts (e.g., other celebrities or general prompts), and (3) a quality loss term that penalizes large changes in the unconditional prediction:
...
## Sample Comparison (Target vs. Trigger) after Erasure

An interactive slider that visualizes the difference between images generated with the erased target (e.g., "a photo of Morgan Freeman") vs. with the hidden ToxE (DISA) trigger in the prompt instead (e.g., "a photo of rhWPSuE").
...
<Figure>
  ...
  <span slot="caption">**ESD vs. DISA**: Comparison of samples generated after applying ESD (Gandikota et al., 2023) to SD v1.4 poisoned with ToxE (DISA). **Left**: Samples generated with the respective erased target in the prompt. **Right**: Samples generated with the hidden ToxE trigger ("rhWPSuE") instead.</span>
</Figure>
...
<Figure>
  ...
  <span slot="caption">**AdvUnlearn vs. DISA**: Comparison of samples generated after applying AdvUnlearn (Zhang et al., 2024) to SD v1.4 poisoned with ToxE (DISA). **Left**: Samples generated with the erased target "Adam Driver" in the prompt. **Right**: Samples generated with the hidden ToxE trigger ("rhWPSuE") instead.</span>
</Figure>
...
<Figure>
  ...
  <span slot="caption">**RECE vs. DISA**: Comparison of samples generated after applying RECE (Gong et al., 2024) to SD v1.4 poisoned with ToxE (DISA). **Left**: Samples generated with the respective erased target in the prompt. **Right**: Samples generated with the hidden ToxE trigger ("rhWPSuE") instead.</span>
</Figure>
